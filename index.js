...
const {useApp, useFrame, useLoaders, usePhysics, useCleanup, useLocalPlayer, useActivate, useInternals, useWear} = metaversefile;
...
export default () => {  
...

  {
    let iphone = null;
    (async () => {
        const u = `${baseUrl}/assets/mobile_phone.glb`;
        ...
        app.glb = i;
        app.add(iphone, tildeGrabGroup);
        iphone.updateMatrixWorld();
        tildeGrabGroup.updateMatrixWorld();

        const physicsId = physics.addGeometry(iphone);
    ...
    (async () => {
      ...
      cone.scene.children[0].material = beamShaderMaterial;
    ...
    useFrame(({timestamp}) => {
      ...
      if (cone) {
        ...
        const dist = group.position.distanceTo(localPlayer.position);
        // ! cone.scene.scale.set(1, dist/2, 1);
        // ! group.lookAt(localPlayer.position);
